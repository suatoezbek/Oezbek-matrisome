---
title: "Gideon Bergheim - P1482"
subtitle: "Version 3 / new DB"
author: "Frank"
date: "`r Sys.Date()`"
output: pdf_document
---
...
```{r echo = FALSE}
setwd("D:/Data/programs/mesoglea_protein_pipeline/GideonBergheim_P1482_newDB_V3")
```
...
data$gene_name[is.na(data$gene_name)] <- data$protein_id[is.na(data$gene_name)]
```
...
if (length(dups) > 0)
{
  ...
  for (dup in dups)
  {
    ...
    for (filen in unique(subset(dup.data, gene_name == dup)$file))
    {
      ...
      {
        sub$ID.count[sub$protein_id == ID] <- length(which(IDs %in% unlist(strsplit(ID, split = "[|]"))))
      }
...
fdata_i <- data %>%
  ...
  group_by(gene_name, file2) %>%
  dplyr::select(gene_name, file, file2) %>%
  unique() %>%
...
for (i in seq_along(conditions$ID))
{
  fc.data[, paste0(conditions$ID[i], ".ctrl.ratio")] <-
    ...
    apply(fc.data[, names.orig], 1, mean, na.rm = TRUE)
...
ggplot(data = sorted_limma, aes(logFC, -log10(pvalue), colour = subset_annotation)) +
  ...
  xlab("log2(fold change)") +
  theme_bw(base_size = 12) +
  scale_color_manual(values = c("gray","#ff7f0e","#ff00ff","#72bce4"))
...
```{r LimmaFoldChangeCorrelation, fig.height = 7}
...
```{r ClusteringPlotting, fig.height = 7}
...
```{r PlotIndividualProteins, eval=F}
